app.controller("DeputyDeanSessionGroupsController", function($scope, $http) {

    $scope.checkRole("DEPUTY_DEAN");

    getSessionGroupsByDeputyDean();
    function getSessionGroupsByDeputyDean() {
        $http({
            method: 'PATCH',
            url: '/getSessionGroupsByDeputyDean',
            params: { groupId: '' }
        }).then(
